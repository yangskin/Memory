...
def test_facade_schema_omits_legacy_project_graph_operation(tmp_path: Path) -> None:
    read_schema = _tool_schema(_make_config(tmp_path), "memory_read")
    assert "project_graph" not in read_schema["properties"]["operation"]["enum"]
    assert "depth" not in read_schema["properties"]
    assert "max_nodes" not in read_schema["properties"]
    assert "max_edges" not in read_schema["properties"]
