...
def test_task_done_uses_context_task_id_without_legacy_graph_settlement(tmp_path, monkeypatch):
    config = _make_config(tmp_path)
    monkeypatch.setattr(
    ...
    assert result["ok"] is True
    assert result["task_state"]["task_id"] == "task-from-context"
    assert "background_task_graph" not in result
