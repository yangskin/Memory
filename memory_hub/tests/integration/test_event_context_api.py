...
def test_authenticated_event_ingest_is_idempotent_and_project_scoped() -> None:
    ...
    payload = {"events": [_event(str(uuid4()))]}
...
def test_tokens_private_metadata_and_secret_redaction_are_enforced() -> None:
    ...
    response = client.post(f"/v1/projects/{project_id}/events/batch", headers={"Authorization": f"Bearer {owner_token}"}, json={"events": [event]})
    assert response.status_code == 200
    ...
    with app.state.session_factory() as session:
        ...
        session.commit()
    context = client.post(f"/v1/projects/{project_id}/context", headers={"Authorization": f"Bearer {visible_token}"}, json={"agent_instance_id": "bob-agent", "include": ["project_activity"]})
    assert context.status_code == 200
    metadata = context.json()["project_activity"][0]["metadata"]
    assert metadata == {"active_files": ["safe.py"]}
    assert "secret_notes" not in metadata