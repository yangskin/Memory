...
def _node(item: GraphNode) -> dict[str, object]:
    return {"id": str(item.id), "type": item.node_type, "key": item.node_key, "name": item.name, "metadata": item.metadata_json or {}, "updated_at": item.updated_at.isoformat()}


def _edge(item: GraphEdge) -> dict[str, object]:
    source_event_ids = item.source_event_ids or []
    return {"id": str(item.id), "source": str(item.source_node_id), "target": str(item.target_node_id), "relation_type": item.relation_type, "confidence": item.confidence, "source_event_ids": source_event_ids, "source_event_count": len(source_event_ids)}


def _graph(session, project_id: str, *, node_limit: int, edge_limit: int, node_ids: set[object] | None = None) -> dict[str, object]:
    ...
    return {"project_id": project_id, "nodes": [_node(item) for item in nodes], "edges": [_edge(item) for item in edges], "updated_at": state.updated_at.isoformat() if state else None, "freshness": {"covers_through_seq": covered, "latest_event_seq": latest, "stale": covered < latest}}


@router.get("/v1/projects/{project_id}/graph")
def graph_snapshot(project_id: str, request: Request, principal: Principal = Depends(require_principal("context:read"))) -> dict[str, object]:
    ...
    with request.app.state.session_factory() as session:
        return _graph(session, project_id, node_limit=500, edge_limit=1000)
...
def graph_query(project_id: str, payload: GraphQueryRequest, request: Request, principal: Principal = Depends(require_principal("context:read"))) -> dict[str, object]:
    ...
    with request.app.state.session_factory() as session:
        ...
        if not filters:
            return _graph(session, project_id, node_limit=payload.max_nodes, edge_limit=payload.max_edges)
        ...
        return _graph(session, project_id, node_limit=payload.max_nodes, edge_limit=payload.max_edges, node_ids=selected)